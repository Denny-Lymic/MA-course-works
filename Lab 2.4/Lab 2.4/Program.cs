...
static void Str_matr_decrypt()
{
    ...
    string[] sos = { "...  ", "---  ", "...  " };
    for (int i = 0; i < sos.Length; i++) Console.Write(sos[i]);
    Console.WriteLine();
...
static void Str_matr_crypt_key()
{
    ...
    code_tbl.Write_matrix();
    string rslt = code_tbl.crypt(word);
    Console.WriteLine("aos9 : " + rslt);
    code_tbl.Res_beep(rslt);

    Console.WriteLine("From beep to aos9");
    string[] sos = { rslt.Substring(0, 5), rslt.Substring(5, 5), rslt.Substring(10, 5), rslt.Substring(15, 5) };
    word = code_tbl.decrypt(sos);
    Console.WriteLine("aos9 decrypt   :" + word);
}
